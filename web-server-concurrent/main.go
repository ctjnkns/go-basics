...
type database struct {
	mu sync.Mutex
	db map[string]dollars //simulate the database with a map
...
func (d *database) add(w http.ResponseWriter, req *http.Request) {
	...
	d.db[item] = dollars(p) //cast the float to the dollars type

	fmt.Fprintf(w, "added %s with price %s\n", item, d.db[item]) //could return a different response code to show we created something
}
...
func (d *database) read(w http.ResponseWriter, req *http.Request) {
	d.mu.Lock()
	defer d.mu.Unlock()
